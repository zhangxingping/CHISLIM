...
import com.weiaibenpao.demo.chislim.service.TeachService;
...
public class TeachModel {
    ...
    private static class TeachHolder{
        private static TeachModel userModel = new TeachModel();//单例对象实例
    }

    private TeachModel(){//private的构造函数用于避免外界直接使用new来实例化对象
        Retrofit retrofit = new Retrofit.Builder()
                .baseUrl(Default.url)
                //转换服务器数据到对象
                .addConverterFactory(GsonConverterFactory.create())
                .build();
        service = retrofit.create(TeachService.class);
    }
